...
class Prompts {
    constructor(application) {
        this.CHAT_GET_KEY_WORDS = "Analyze the text below and extract the most important keywords. Don't include @ in the keywords. Ensure no word is repeated in the output. Format the response strictly as:\nkeyword1|keyword2|...\nText: {prompt}";
        this.CHAT_GET_SYNONYMS = "Get up to two different synonyms for each of the following words and make one list from all of them in format word1|word2|word3.\nWords: {keywords} ";
        this.CHAT_EDIT_TEXT = `Modify the following original code according to the instructions. Output only the modified code. No explanations.\n\ninstructions:\n{instructions}\n\noriginal code:\n{originalText}\n\nmodified code:`;
        this.CHAT_GET_SUMMARY = "Summarize the following conversation between the user and AI assistant. Focus on key decisions, code snippets, requirements, and important context. Keep the summary concise (under 300 words) and preserve technical details.";
        this.CREATE_GIT_DIFF_COMMIT = `Please generate a readable and concise git commit message based on the file changes.
...
**OUTPUT:**:
`;
        this.TOOLS_SYSTEM_PROMPT_ACTION = `You are an agent for software development - please keep going until the user’s query is completely resolved, before ending your turn and yielding back to the user. 
...
`;
        this.TOOLS_SYSTEM_PROMPT_PLANNING = `You are an expert in planning. You are working in a planning mode and just plan. You do not take actions.`;
        this.TOOLS_ANALYSE_GOAL = `
...
`;
    ...
    replaceOnePlaceholder(template, key, replacement) {

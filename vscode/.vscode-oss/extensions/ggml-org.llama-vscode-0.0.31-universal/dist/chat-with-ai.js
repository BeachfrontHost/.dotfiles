...
class ChatWithAi {
    constructor(application) {
        this.sentContextChunks = [];
        this.showChatWithTools = async (context) => {
            let query = undefined;
        ...
        this.closeChatWithAi = (withContext) => {
            if (withContext) {
        ...
        this.showChatWithAi = async (withContext, context, aiInitialExtraContext = "") => {
            ...
            if (!targetUrl) {
                const shouldSelectModel = await utils_1.Utils.showUserChoiceDialog("Select a chat or tools model run by llama-server or an env with chat or tools model run on llama-server to chat with AI.", "Select");
            ...
            if (editor) {
                ...
                if (queryToSend.length > 0)
                    queryToSend = "Explain the following source code: " + queryToSend;
            }
            if (query) {
                queryToSend = query;
            }
            if (!aiPanel) {
                ...
                aiPanel = vscode.window.createWebviewPanel(webviewIdentifier, panelTitle, vscode.ViewColumn.Three, // Editor column to show the Webview
                {
                    enableScripts: true,
                ...
                if (withContext)
                    this.askAiWithContextPanel = aiPanel;
                else
                    this.askAiPanel = aiPanel;
                if (aiPanel)
                    context.subscriptions.push(aiPanel);
                ...
                if (query)
                    extraCont += await this.prepareRagContext(query);
                setTimeout(async () => {
                    if (aiPanel)
                        aiPanel.webview.postMessage({ command: 'setText', text: queryToSend, context: extraCont });
        ...
        this.prepareRagContext = async (query) => {
            let extraCont = "";
            ...
            if (chunksToSend.length > 0)
                extraCont = this.app.chatContext.getContextChunksInPlainText(chunksToSend);

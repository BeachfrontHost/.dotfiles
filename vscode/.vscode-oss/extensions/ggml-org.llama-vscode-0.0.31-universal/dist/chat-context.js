...
class ChatContext {
    constructor(application) {
        ...
        this.getRagContextChunks = async (prompt) => {
        ...
        this.getRagFilesContext = async (prompt) => {
        ...
        this.getContextChunksInPlainText = (chunksToSend) => {
        ...
        this.cosineSimilarityRank = async (query, chunkEntries, topN) => {
        ...
        this.cosineSimilarity = async (a, chunk) => {
        ...
        this.isImageOrVideoFile = (filename) => {
        ...
        this.getFileProperties = (uri) => {
            return this.filesProperties.get(uri);
        };
        this.addDocument = async (uri, content) => {
            try {
                ...
                for (let i = 0; i < lines.length; i += this.app.configuration.rag_max_lines_per_chunk) {
                    ...
                    if (this.entries.size >= this.app.configuration.rag_max_chunks)
                        break;
                    this.nextEntryId++;
                }
            }
        ...
        this.getFilesRespectingGitignore = async () => {
            ...
            await Promise.all(gitignoreUris.map(async (uri) => {
                try {
                    ...
                    igMap.set(dir, (0, ignore_1.default)().add(content.toString()));
                }
                catch (error) {
            ...
            async function traverse(dirUri) {
                ...
                for (const [name, type] of entries) {
                    ...
                    if (type === vscode.FileType.Directory) {
                        if (entryUri.toString().toLowerCase().endsWith(".git"))
                            continue;
    ...
    async indexWorkspaceFiles() {
        try {
            ...
            if (this.app.configuration.rag_max_files <= 0)
                return;
            ...
            await vscode.window.withProgress(progressOptions, async (progress, token) => {
                ...
                for (const file of files) {
                    ...
                    if (this.isImageOrVideoFile(file.toString()))
                        continue;
                    try {
